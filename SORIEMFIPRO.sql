...
/****************************************************************/
	...
	if @Tip_Proces = @Str_A begin
		...
		select @Str_NumPer = (select DISTINCT(Rib_NumPer) from SORIB noholdlock where Rib_Numero = @Ref_NumRib)
		select @Int_NumSol = (select DISTINCT(Rib_NumSol) from SORIB noholdlock where Rib_Numero = @Ref_NumRib)
		...
		if @Int_NumSol > @Int_Cero begin
			...
			if exists (select Rib_Numero from SORIB noholdlock where Rib_NumPer = @Str_NumPer and Rib_NumSol = @Int_Cero) begin
				...
				delete from SORIEMFI where Ref_NumRib = @Int_RibBas

				insert into SORIEMFI (
						Ref_NumRib,		Ref_NumPer,		Ref_PriAct,		Ref_TiReNe,
						Ref_TiReOt,		Ref_PeSiRf,		Ref_Activo,		NumTransac,
						Transaccio,		Usuario,		FechaSis,		SucOrigen,
						SucDestino)
				select	@Int_RibBas,	Ref_NumPer,		Ref_PriAct,		Ref_TiReNe,
						Ref_TiReOt,		Ref_PeSiRf,		Ref_Activo,		@NumTransac,
						@Transaccio,	@Usuario,		@FechaSis,		@SucOrigen,
						@SucDestino
				from SORIEMFI noholdlock
				where Ref_NumRib = @Ref_NumRib
				  and Ref_Activo = @Int_Uno
			end
		end
	end
