...
as
/****************************************************************/
/** Descripcion :	Procesamiento de registros de Productos		*/
/**					y Servicios asociados a RIB					*/
/****************************************************************/
/** Creo:		Victor Osorio									*/
/** Fecha:		02/05/2017                               		*/
/** Help:		929417 					 						*/
/****************************************************************/
	...
	if @Tip_Proces = @Str_A begin
		...
		select @Str_NumPer = (select DISTINCT(Rib_NumPer) from SORIB noholdlock where Rib_Numero = @Rps_NumRib)
		select @Int_NumSol = (select DISTINCT(Rib_NumSol) from SORIB noholdlock where Rib_Numero = @Rps_NumRib)
		...
		if @Int_NumSol > @Int_Cero begin
			...
			if exists (select Rib_Numero from SORIB noholdlock where Rib_NumPer = @Str_NumPer and Rib_NumSol = @Int_Cero) begin
				...
				delete from SORIPRSE where Rps_NumRib = @Int_RibBas

				insert into SORIPRSE (
						Rps_NumRib,		Rps_ProSer,		Rps_MarCom,		Rps_PoVeIn,
						Rps_Activo,		NumTransac,		Transaccio,		Usuario,
						FechaSis,		SucOrigen,		SucDestino)
				select	@Int_RibBas,	Rps_ProSer,		Rps_MarCom,		Rps_PoVeIn,
						Rps_Activo,		@NumTransac,	@Transaccio,	@Usuario,
						@FechaSis,		@SucOrigen,		@SucDestino
