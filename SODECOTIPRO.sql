...
** REFERENCIAS:															****
****************************************************************************
** Modifico:	jonathan.nicio@banregio.com								****
** Fecha:		20/julio/2026											****
** KeyJira:		TCELCV-37936											****
** Descripción:	Agrega validaciones a duplicados						****
****************************************************************************
...
select	@Pro_TipMov = 'P00001'	--Producto, Personalidad Fiscal y Tipo de Movimiento
...
if @Var_Contin = @Bit_Si begin
	...
	delete SOHISTMC	--Eliminar registros de tablas de Historia. Para casos de reproceso.
	...
	insert into SOTMPPRP(
		Prp_NumTra,	Prp_Produc,	Prp_TipCue,	Prp_Moneda,	Prp_PerFis,
...
select 	@Pro_TipMov = 'P00002'	-- Cuentas
...
if @Var_Contin = @Bit_Si begin
	...
	insert into #CuentasAct	(
		Cua_Cuenta,	Cua_CliEnt,	Cua_TipCue,	Cua_Moneda,	Cua_ClClEn,
		Cua_SucEnt)
...
--Codigo para ejecucion de Proceso de Preparacion de Productos y Cuentas
select	@Pro_TipMov = 'A00001'	--Configuraciones a Nivel Producto
...
while (@Reg_CoTiMo <= @Reg_TipMov) begin --Ejecutar cada Comision
	...
	if @Var_Contin = @Bit_Si begin
		...
		insert into SOTMPCUL(
			...
			Cul_Priori)
				select	distinct	Cue_Numero,	Cue_Tipo,	Cue_Moneda,	Prp_Produc,	Prp_PerFis,
		...
		insert into #ConfiguracionProd(
			Cop_Cuenta,	Cop_TipMov,	Cop_NivEnt,	Cop_Vigenc,	Cop_Produc,
			Cop_PerFis,	Cop_Elemen,	Cop_Priori,	Cop_PrPeCo,	Cop_PrTiMo)
			select	distinct	Cul_Cuenta,	Cul_TiMoCa,	Cul_NivEnt,	convert(tinyint, Cul_Vigenc),	Cul_Produc,
		...
		insert into SOTMPCCN
			...
			select distinct	cp.Cop_Cuenta,	cp.Cop_TipMov,	cp.Cop_NivEnt,	cp.Cop_Vigenc,	cp.Cop_Produc,
					cp.Cop_PerFis,	cp.Cop_Elemen,	tma.Tma_Numero,	tma.Tma_Aplica, tma.Tma_Termin,
					cp.Cop_Priori, @Bit_Si,	@Bit_Si
			from #ConfiguracionProd cp
			inner join SOTIMOAS tma noholdlock			--Tipos de Movimientos en cada Prod/PerFis dentro de cada Configuración
					on tma.Tma_PrPeCo	=	cp.Cop_PrPeCo
					and tma.Tma_PrTiMo	=	cp.Cop_PrTiMo
					and tma.Tma_Activo	=	@Bit_Si
			where not exists (
				select 1
				from SOTMPCCN exi noholdlock
				where exi.Ccn_Cuenta = cp.Cop_Cuenta
				  and exi.Ccn_TipMov = cp.Cop_TipMov
				  and exi.Ccn_NivEnt = cp.Cop_NivEnt
				  and exi.Ccn_Vigenc = cp.Cop_Vigenc
				  and exi.Ccn_Produc = cp.Cop_Produc
				  and exi.Ccn_PerFis = cp.Cop_PerFis
				  and exi.Ccn_Elemen = cp.Cop_Elemen
				  and exi.Ccn_TiMoAs = tma.Tma_Numero
			)
		-- En caso de error terminar con error
		if @@error <> @Ent_Cero begin
			rollback transaction
		...
		insert into SOTMPCCN
			...
			Ccn_PerFis,		Ccn_Elemen, Ccn_TiMoAs,	Ccn_Aplica, Ccn_Termin,
			Ccn_Priori, 	Ccn_AplCal, Ccn_Activo)
			select	distinct	cmo.Cmo_Cuenta,
					substring('000000', 1, 6 - len(rtrim(convert(CHAR(6), cmo.Cmo_TipMov)))) + rtrim(convert(CHAR(6), cmo.Cmo_TipMov)),
					@Ent_NivPro,	@Ent_VigMod,	ctm.Ctm_Numero,
					pfc.Pfc_PerFis, 	ctm.Ctm_Numero, 	tma.Tma_Numero,	tma.Tma_Aplica,	tma.Tma_Termin,
					ppt.Ppt_Priori, 	@Bit_No, 		@Bit_Si
			from	CHTMPCMO cmo noholdlock
			inner join CHCUENTA cue noholdlock
				on cue.Cue_Numero = cmo.Cmo_Cuenta
			inner join CLCLIENT cli noholdlock
				on cli.Cli_Numero = cue.Cue_Client
			inner join SOPEFICL pfc noholdlock
				on pfc.Pfc_NuPeCl = cli.Cli_Tipo
				and pfc.Pfc_ActEmp = cli.Cli_ActEmp
			inner join SOPRPEFI ppf noholdlock
				on ppf.Ppf_Produc = @Ent_ProMod
				and ppf.Ppf_PerFis = pfc.Pfc_PerFis
				and ppf.Ppf_Activo = @Bit_Si
			inner join SOPRTIMO ptm noholdlock
				on ptm.Ptm_PrPeFi = ppf.Ppf_Numero
				and ptm.Ptm_TipMov = cmo.Cmo_TipMov
				and ptm.Ptm_Activo = @Bit_Si
			inner join SOPRPETI ppt noholdlock
				on ppt.Ppt_PrTiMo = ptm.Ptm_Numero
				and ppt.Ppt_NivEnt = @Ent_NivPro
				and ppt.Ppt_Activo = @Bit_Si
			inner join SOCOTIPR ctp noholdlock
				on ctp.Ctp_Produc = @Ent_ProMod
				and ctp.Ctp_Activo = @Bit_Si
			inner join SOCOTIMO ctm noholdlock
				on ctm.Ctm_Numero = ctp.Ctp_CoTiMo
				and ctm.Ctm_Vigenc = @Bit_No
				and ctm.Ctm_Activo = @Bit_Si
			inner join SOPRPECO ppc noholdlock
				on ppc.Ppc_CoTiMo = ctm.Ctm_Numero
				and ppc.Ppc_PrPeFi = ppf.Ppf_Numero
				and ppc.Ppc_Activo = @Bit_Si
			inner join SOTIMOAS tma noholdlock
				on tma.Tma_PrPeCo = ppc.Ppc_Numero
				and tma.Tma_PrTiMo = ptm.Ptm_Numero
				and tma.Tma_Activo = @Bit_Si
			where	cmo.Cmo_TipMov = convert(int, @Pro_TipMov)
			  and not exists (
				select 1
				from SOTMPCCN exi noholdlock
				where exi.Ccn_Cuenta = cmo.Cmo_Cuenta
				  and exi.Ccn_TipMov = substring('000000', 1, 6 - len(rtrim(convert(CHAR(6), cmo.Cmo_TipMov)))) + rtrim(convert(CHAR(6), cmo.Cmo_TipMov))
				  and exi.Ccn_NivEnt = @Ent_NivPro
				  and exi.Ccn_Vigenc = @Ent_VigMod
				  and exi.Ccn_Produc = ctm.Ctm_Numero
				  and exi.Ccn_PerFis = pfc.Pfc_PerFis
				  and exi.Ccn_Elemen = ctm.Ctm_Numero
				  and exi.Ccn_TiMoAs = tma.Tma_Numero
			)
		--En caso de error hacer rollback
		if @@error <> @Ent_Cero begin
